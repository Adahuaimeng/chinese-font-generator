...
class FontGeneratorPartial:
    ...
    def generate(self, ttf_path, size, font_name, chars, output_dir=None):
        ...
        seen = set()
        for ch in chars:
            if ch not in seen and ord(ch) >= 0x4E00 and ord(ch) <= 0x9FA5:  # 只保留GB2312汉字
        ...
        with open(h_path, 'w', encoding='utf-8') as f:
            ...
            f.write('    bool hasChar(const char* ch);  // 检查是否包含某字\n')
            f.write('    \n')
            f.write('private:\n')
            f.write('    int findCharIndex(const char* ch);  // 线性查找字符索引\n')
            f.write('    void* _tft;\n')
        ...
        with open(cpp_path, 'w', encoding='utf-8') as f:
            ...
            f.write('    if(index < 0) return;  // 字符不在字库中\n')
            f.write('    \n')
            ...
            f.write('    \n')
            f.write('    const int maxLines = 50;\n')
            f.write('    int lineStarts[100];  // 存储每行起始索引（字符位置）\n')
            ...
            f.write('        int drawX = centerX - width / 2;\n')
        ...
        print(f'\n新增函数:')
        print(f'  {obj_name}.hasChar("中");  // 检查字库是否包含某字')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GB2312 部分字字库生成器')
